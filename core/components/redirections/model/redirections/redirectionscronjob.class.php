...
class RedirectionsCronjob extends Redirections
{
    /**
     * @access protected.
     * @var Boolean.
        ...
        switch ($type) {
            ...
            case 'end':
                if ($log = $this->setLogFile()) {
                    if ((int) $this->modx->getOption('redirections.log_send') === 1 && !$this->getDebugMode()) {
    ...
    protected function sendLogFile($log)
    {
        ...
        if ($mail) {
            ...
            $mail->set(modMail::MAIL_SUBJECT, $this->modx->getOption('site_name') . ' | Redirections clean');

            $mail->set(modMail::MAIL_BODY, $this->modx->getOption('redirections.log_body', null, 'Log file is attached to this email.'));

            $mail->mailer->AddAttachment($log);

            $emails = explode(',', $this->modx->getOption('redirections.log_email', null, $this->modx->getOption('emailsender')));
    ...
    protected function cleanFiles()
    {
        ...
        $lifetime = $this->modx->getOption('redirections.log_lifetime', null, 7);
    ...
    protected function clean()
    {
        ...
        $criteria = $this->modx->newQuery('RedirectionsRedirect');

        $criteria->where([
             'active'        => 2,
             'last_visit:<'  => date('Y-m-d', strtotime('-' . $this->modx->getOption('redirections.clean_days', null, 30) .' days'))
         ]);

        foreach ($this->modx->getCollection('RedirectionsRedirect', $criteria) as $object) {
            $object->remove();
