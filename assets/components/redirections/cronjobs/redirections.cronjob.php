...
if (!isset($options['hash']) || $options['hash'] !== $modx->getOption('redirections.cronjob_hash')) {
    $modx->log(modX::LOG_LEVEL_INFO, 'ERROR:: Cannot initialize service, no valid hash provided.');

    exit();
}

$service = $modx->getService('redirectionscronjob', 'RedirectionsCronjob', $modx->getOption('redirections.core_path', null, $modx->getOption('core_path').'components/redirections/') . 'model/redirections/');

if ($service instanceof RedirectionsCronjob) {
